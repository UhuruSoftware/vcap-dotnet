...
{
    [TestClass]
    public class UhurufsServicesTest
    {
        private string target;
        private string username = "dbtest2@uhurucloud.net";
        ...
        public void TC001_DatabaseCreate()
        {
            ...
            try
            {
                RawSystemService systemService = cloudConnection.SystemServices.First(ss => ss.Vendor == "uhurufs");
        ...
        public void TC003_3Secquential()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                try
                {
                    cloudConnection.CreateProvisionedService(cloudConnection.SystemServices.FirstOrDefault(ss => ss.Vendor == "uhurufs"), serviceName, true);
                    Thread.Sleep(10000);
                    ICollection<ProvisionedService> services = cloudConnection.ProvisionedServices;
                    foreach (ProvisionedService svc in services)
                    {
