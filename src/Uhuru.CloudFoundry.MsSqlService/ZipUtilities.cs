        ...
        public static void SetupZlib()
        {
            ...
            lock (zlibLock)
            {
                ...
                if (IntPtr.Size == 8)
                {
                    ...
                    stream = asm.GetManifestResourceStream("Uhuru.CloudFoundry.MSSqlService.lib.7z64.dll");
                }
                else
                {
                    libraryPath = Path.Combine(Path.GetDirectoryName(Assembly.GetAssembly(typeof(SevenZipExtractor)).Location), @"7z86.dll");
                    stream = asm.GetManifestResourceStream("Uhuru.CloudFoundry.MSSqlService.lib.7z86.dll");
                }
