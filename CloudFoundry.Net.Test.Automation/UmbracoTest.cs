// -----------------------------------------------------------------------
// <copyright file="UmbracoTest.cs" company="Uhuru Software">
...
namespace CloudFoundry.Net.Test.Automation
{
    ...
    class UmbracoTest
    {
        ...
        private void PushUmbraco(string appName, string serviceName, string deploymentDir, string url)
        {
            ...
            string targetDir = Helper.CopyFolderToTemp(deploymentDir);
            lock (lck)
            {
                foldersCreated.Add(targetDir);
            }
            Helper.UpdateWebConfigKey(targetDir + "\\Web.config", "umbracoDbDSN", "{mssql#" + serviceName + "}");
